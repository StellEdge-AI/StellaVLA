...
| `/opt/conda/envs/libero` | 3.10 | LIBERO — robosuite 1.4.1, mujoco 3.2.3 |
| `/opt/conda/envs/libero_plus` | 3.10 | LIBERO-Plus — robosuite 1.4.0 + its 6.4 GB assets |
| `/app/VLA-Arena/envs/base/.venv` | 3.11 | VLA-Arena — robosuite 1.5.1, from its own uv lock |
...
Weights and demo packs (~19.6 GB) are not baked in. Fetch them once into a
host directory and mount it at `/data`:
...
GPU 0. Every benchmark writes under `/data`, so results survive the container.
...
| libero | 2.11.0+cpu | 0.26.0+cpu | — | 3.2.3 | 1.4.1 | 1.24.4 |
| libero_plus | 2.11.0+cpu | 0.26.0+cpu | 4.21.1 | 3.2.3 | 1.4.0 | 1.22.4 |
| vla_arena | from its uv lock | | | 3.5.0 | 1.5.1 | 1.26.4 |
