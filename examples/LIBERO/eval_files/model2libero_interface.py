...
from examples.common.adaptive_ensemble import AdaptiveEnsembler
from typing import Dict
from pathlib import Path

from stellavla.model.tools import read_mode_config
...
class ModelClient:
    ...
    def step(
        ...
        **kwargs
        ...
        # `num_actions_to_execute` (kwarg) lets the caller use only the first N
        # actions of each predicted chunk before re-querying the model. Default
        # = full action_chunk_size (use the entire predicted chunk). Setting it
        # smaller (e.g. 4 of 8) increases re-query frequency and reduces drift
        # from stale chunk tails.
        action_chunk_size = self.action_chunk_size
        exec_period = int(kwargs.get("num_actions_to_execute") or action_chunk_size)
        if exec_period <= 0 or exec_period > action_chunk_size:
            exec_period = action_chunk_size

        if step % exec_period == 0:
        ...
        raw_actions = self.raw_actions[step % exec_period][None]
