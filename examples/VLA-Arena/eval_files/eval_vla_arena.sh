...
# eval_vla_arena.sh
#
# Batch evaluation of a stellavla policy on one or more VLA-Arena task suites
...
# Run from the stellavla root directory.

###########################################################################################
# === Paths — set these in the environment, or edit the defaults here ===
: "${VLA_ARENA_HOME:=/path/to/VLA-Arena}"       # Root of the VLA-Arena repository
: "${VLA_ARENA_python:=python}"                 # Python env that has vla_arena installed
export VLA_ARENA_HOME VLA_ARENA_python

export STELLAVLA_HOME=$(pwd)
# The repo ROOT goes on the path: the eval imports `vla_arena.vla_arena`, so
# pointing at <root>/vla_arena instead shadows the outer package and breaks it.
export PYTHONPATH=${VLA_ARENA_HOME}:${PYTHONPATH}
...
Batch evaluation of a stellavla policy server against VLA-Arena task suites.
