...
Evaluates a stellavla policy in VLA-Arena simulation benchmark environments via
...
class Args:
    ...
    long_horizon_max_steps: int = 600
    """Per-episode step budget for long_horizon L1/L2 (multi-step compound tasks).
    Default 600; raise (e.g. 1200) to test whether 0% SR is a step-budget cliff
    vs a genuine multi-step-chaining limitation. Other suites use 300."""
    ...
    wandb_project: str = "stellavla_VLA_Arena"
...
def _run_episode(
    ...
    # Reset env and set initial state. Robosuite can raise RandomizationError
    # ("cannot place all objects") on a crowded scene (e.g. distractor_dynamic
    # L2) — object placement is RNG-based and the failure is intermittent, so
    # retry the reset a few times before giving up (otherwise one unlucky reset
    # crashes the entire suite eval and the cell is reported as missing).
    obs = None
    for _reset_attempt in range(25):
        try:
            env.reset()
            obs = (
                env.set_init_state(initial_state)
                if initial_state is not None
                else env.get_observation()
            )
            break
        except Exception as _reset_exc:  # noqa: BLE001
            _msg = str(_reset_exc).lower()
            _is_place_fail = (
                "randomizationerror" in type(_reset_exc).__name__.lower()
                or "place all objects" in _msg
                or "cannot place" in _msg
            )
            if _is_place_fail and _reset_attempt < 24:
                logging.getLogger(__name__).warning(
                    f"[{suite_name} L{task_level} t{task_id}] env.reset placement "
                    f"failed (attempt {_reset_attempt + 1}/25), retrying: {_reset_exc}"
                )
                continue
            raise
    ...
    # Determine max steps. long_horizon L1/L2 are multi-step compound tasks that
    # need a bigger budget (configurable to probe the step-budget vs chaining
    # question); everything else is single-goal and 300 is ample.
    if suite_name == "long_horizon" and task_level >= 1:
        max_steps = args.long_horizon_max_steps
    else:
        max_steps = 300
    ...
    step = 0
    replay_images = []
    full_actions = []   # executed delta actions per step — failure viz
    cost = 0.0
    success = False
    ...
    try:
        while t < max_steps + args.num_steps_wait:
            ...
            # Rotate 180° to match training pre-processing (same convention as the
            # LIBERO eval). The model was trained on TWO cameras (video.primary_image
            # = agentview, video.wrist_image = eye_in_hand), so feed both in that order.
            ...
            example_dict = {
                ...
                # Fields the server uses to look up this task's demo: the
                # instruction text plus (suite, level, task_id). `state` reuses
                # the raw axis-angle end-effector state computed above.
                "suite": suite_name,
                "level": task_level,
                "task_id": int(task_id),
                "episode_id": f"{suite_name}_L{task_level}_t{task_id}_e{episode_idx}",
...
def eval_vla_arena(args: Args) -> dict:
    ...
    if args.use_wandb:
        ...
        run_id = f"EVAL-VLA_Arena-stellavla-{date_str}"
        if args.job_name:
    ...
    for suite_name in suite_names:
        ...
        for task_id in tqdm.tqdm(range(num_tasks), desc=f"{suite_name} tasks"):
            ...
            # Build environment. Robosuite raises RandomizationError ("cannot
            # place all objects") when CONSTRUCTING a crowded scene (e.g.
            # distractor_dynamic L2) — placement is RNG-based, so retry the
            # build (a fresh construction re-samples placement and succeeds).
            env = None
            for _env_attempt in range(25):
                try:
                    ...
                    _emsg = str(_env_exc).lower()
                    _place_fail = (
                        "randomizationerror" in type(_env_exc).__name__.lower()
                        or "place all objects" in _emsg
                        or "cannot place" in _emsg
                    )
                    if _place_fail and _env_attempt < 24:
                        logging.getLogger(__name__).warning(
                            f"[L{task_level} t{task_id}] env build placement failed "
                            f"(attempt {_env_attempt + 1}/25), retrying: {_env_exc}"
                        )
                        continue
                    raise
