...
class ModelClient:
    """
    StellaVLA WebSocket policy client adapted for VLA-Arena environments.

    Connects to the stellavla deployment server and provides step-by-step
    ...
    def _resize_image(self, image: np.ndarray) -> np.ndarray:
        # Honor a configurable target size. Accepts an int (square, e.g. 256) or
        # a [w, h] list/tuple. Default stays [224, 224]. Setting this to 256
        # matches the VLA-Arena env render resolution and avoids a lossy
        # 256->224->256 round-trip vs the model's training resolution.
