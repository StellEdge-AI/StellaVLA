...
from stellavla.model.tools import auto_get_trainable_modules

from stellavla.model.framework.share_tools import read_mode_config
from stellavla.utils import initialize_overwatch
from stellavla.model.framework.share_tools import dict_to_namespace
from stellavla.model.framework.__init__ import build_framework
...
class baseframework(PreTrainedModel):
    ...
    def from_pretrained(
        ...
        **kwargs,
        ...
        try:
            # Filter out norm stats saved during training (loaded separately via norm_stats.json)
            model_state_dict = {k: v for k, v in model_state_dict.items() if k in model_keys}
        ...
        # Restore norm stats so predict_action() can un-normalize correctly.
        # set_norm_stats() registers action_norm_mean/action_norm_std as buffers
        # and sets _has_norm_stats=True, which predict_action() checks.
        # norm_stats here is the dict loaded from dataset_statistics.json.
        if norm_stats and hasattr(FrameworkModel, "set_norm_stats"):
            try:
                from stellavla.utils.norm_stats import NormStats
                FrameworkModel.set_norm_stats(NormStats.from_dict(norm_stats))
            except Exception as e:
                logger.warning(f"Could not restore norm stats via set_norm_stats: {e}")
